pipeline {
    ...
    stages {
        ...
        stage('Deploy to Target') {
            steps {
                sh '''
                    sshpass -p $SSH_PASSWORD ssh -o StrictHostKeyChecking=no $SSH_USER@$TARGET_HOST "sudo -S dnf install -y epel-release && sudo -S dnf install -y https://rpms.remirepo.net/enterprise/remi-release-9.rpm && sudo -S dnf module reset -y php && sudo -S dnf module enable -y php:remi-7.4 <<< $SSH_PASSWORD && sudo -S dnf clean all <<< $SSH_PASSWORD && sudo -S dnf update -y <<< $SSH_PASSWORD && sudo -S dnf install -y --skip-broken httpd php php-mysqlnd php-pecl-zip mariadb-server zip unzip <<< $SSH_PASSWORD"
